...
for (const [label, value] of Object.entries({ release, brandRelease, stabilityRelease, contentRelease, uiRelease, engagementRelease, timelineRelease, weeklyRelease })) {
...
if (!index.includes(`v2-app.css?weekly=${weeklyRelease}`)) throw new Error('canonical stylesheet is not weekly-versioned');
if (!index.includes(`src/v2/main.mjs?weekly=${weeklyRelease}`)) throw new Error('canonical application entry is not weekly-versioned');
...
const sourceStyles = ['v2-core.css', 'v2-features.css', 'v2-responsive.css', 'v2-quality.css', 'v2-editorial.css', 'v2-stabilization.css', 'v2-stabilization-a11y.css', 'v2-m4.css', 'v2-m4-timeline.css', 'v2-m4-weekly.css'];
...
for (const layer of ['core', 'features', 'responsive', 'quality', 'editorial', 'stability', 'accessibility', 'engagement', 'timeline', 'weekly']) {
...
if (!cssEntry.includes(`v2-m4-weekly.css?weekly=${weeklyRelease}`)) throw new Error('Weekly Vibe CSS is not version-locked');
...
if (!mainSource.includes(`./ui/app.mjs?weekly=${weeklyRelease}`)) throw new Error('canonical app import is not weekly-versioned');
...
if (!appSource.includes(`import('./screens/weekly.mjs?weekly=${weeklyRelease}')`)) throw new Error('weekly route is not weekly-versioned');
...
if (!timelineActionSource.includes('restore-profile-snapshot') || !timelineActionSource.includes('clear-profile-history')) throw new Error('timeline actions are incomplete');
...
if (buildInfo.entry !== `/src/v2/main.mjs?weekly=${weeklyRelease}`) throw new Error('canonical entry release contract is inconsistent');
if (buildInfo.styleEntry !== `/v2-app.css?weekly=${weeklyRelease}`) throw new Error('canonical style release contract is inconsistent');
...
console.log(`Core ${release} + brand ${brandRelease} + stability ${stabilityRelease} + content ${contentRelease} + UI ${uiRelease} + engagement ${engagementRelease} + timeline ${timelineRelease} + weekly ${weeklyRelease} verified at ${root}`);
