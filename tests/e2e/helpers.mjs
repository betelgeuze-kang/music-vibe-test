...
export async function completeProfile(page, option = 'a', options = {}) {
  ...
  let firstQuestionCompletedOnHome = false;
  ...
  if (start) {
    ...
    if (await homeChoice.isVisible().catch(() => false)) {
      ...
      firstQuestionCompletedOnHome = true;
  ...
  const firstQuestionNumber = firstQuestionCompletedOnHome ? 2 : 1;
  for (let questionNumber = firstQuestionNumber; questionNumber <= 10; questionNumber += 1) {
