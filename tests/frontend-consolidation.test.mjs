...
assert(index.includes('src/v2/main.mjs?weekly=m4w1'));
...
assert(main.includes('./ui/app.mjs?weekly=m4w1'));
assert(app.includes("import('./screens/profile.mjs?timeline=m4t1')"));
assert(app.includes("import('./screens/weekly.mjs?weekly=m4w1')"));
...
assert.equal(buildInfo.entry, '/src/v2/main.mjs?weekly=m4w1');
assert.equal(buildInfo.styleEntry, '/v2-app.css?weekly=m4w1');
...
console.log('Frontend consolidation, feedback, timeline, and Weekly Vibe layering checks passed.');
