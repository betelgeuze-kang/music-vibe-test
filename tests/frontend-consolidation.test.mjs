...
assert(index.includes('src/v2/main.mjs?frontend=fq1'));
...
assert(main.includes('./ui/app.mjs?frontend=fq1'));
...
assert.equal(buildInfo.entry, '/src/v2/main.mjs?frontend=fq1');
assert.equal(buildInfo.styleEntry, '/v2-app.css?frontend=fq1');
...
console.log('Frontend consolidation, feedback, timeline, Weekly Vibe, and FQ1 layering checks passed.');
